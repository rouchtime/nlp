	...
	public static void main(String[] args) throws IOException {
		...
		for (String label : corpus.labels()) {
			for (News news : corpus.newsFromLabel(label)) {
				...
				String first_sents = corpus.sents(news.getTitle()).get(0);
				...
				try {
					String secondSent = corpus.sents(news.getTitle()).get(1);
					sb.append(secondSent);
					String thirdSent = corpus.sents(news.getTitle()).get(2);
					sb.append(thirdSent);
