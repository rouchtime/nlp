...
public class EmTwentyNewsgroups {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int numSupervisedItems : new Integer[] { 1, 2, 4, 8, 16, 32, 64, 128, 256, 512 }) {
			...
			System.out.println("     ---------------------");
			System.out.printf(
					"#Sup=%4d  Supervised mean(acc)=%5.3f sd(acc)=%5.3f   EM mean(acc)=%5.3f sd(acc)=%5.3f     %10s\n\n",
					numSupervisedItems, Statistics.mean(accs), Statistics.standardDeviation(accs),
					Statistics.mean(accsEm), Statistics.standardDeviation(accsEm),
