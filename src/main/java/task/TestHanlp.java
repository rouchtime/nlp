...
public class TestHanlp {
	public static void main(String[] args) throws IOException {
		// Nature.create("表情符号");
		// for(String express : FileUtils.readLines(new
		// File("D://face_expression.txt"),"utf-8")) {
		// CustomDictionary.insert(express.trim(), "表情符号 1024");
		// }
		// StandardTokenizer.SEGMENT.enablePartOfSpeechTagging(true); // 依然支持隐马词性标注
		// String text = "苹果电脑可以运行开源阿尔法狗代码吗，(^人^)";
		// for(String token :
		// HanLPTokenizerFactory.getIstance().tokenizer(text.toCharArray(), 0,
		// text.length())) {
		...
		for (String line : lines) {
			...
			for (String token : HanLPKeyWordTTokenizerFactory.getIstance(5).tokenizer(text.toCharArray(), 0, text.length())) {
...
//				if (token.split("/")[1].matches("n.*")) {
					FileUtils.write(new File("D://title_result_keyword.txt"), String.format("%s\t%s\n", url, token),
							"utf-8", true);
//				}
			}
